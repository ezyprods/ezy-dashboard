import { Settings, Palette, Database, Shield } from 'lucide-react';
...
export default function SettingsPage() {
  return (
    <div className="flex flex-col items-center justify-center h-[80vh] animate-fade-in text-center px-4">
      ...
      <h1 className="text-3xl font-bold text-text-primary mb-3">Configuración (Próximamente)</h1>
      <p className="text-text-secondary max-w-md mx-auto mb-8 text-lg">
        El centro de ajustes para personalizar tu estudio, gestionar integraciones de IA y administrar tu cuenta.
      </p>

      <div className="flex flex-wrap justify-center gap-4">
        <div className="glass px-6 py-4 rounded-2xl flex items-center gap-3 border border-border">
          <Palette className="w-5 h-5 text-purple-400" />
          <span className="font-medium text-text-secondary">Apariencia</span>
        </div>
        <div className="glass px-6 py-4 rounded-2xl flex items-center gap-3 border border-border">
          <Database className="w-5 h-5 text-blue-400" />
          <span className="font-medium text-text-secondary">Respaldos Drive</span>
        </div>
        <div className="glass px-6 py-4 rounded-2xl flex items-center gap-3 border border-border">
          <Shield className="w-5 h-5 text-emerald-400" />
          <span className="font-medium text-text-secondary">Seguridad</span>
        </div>
      </div>
    </div>
  );
